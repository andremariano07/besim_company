...
def backup_banco(): 
    try: 
        ...
        if os.path.exists(DB_PATH): 
            ...
            destino = os.path.join(GOOGLE_DRIVE_BACKUP, "banco", f"besim_company_{agora}.db") 
...
def backup_pdf(caminho_pdf: str, tipo: str): 
    try: 
        ...
        if os.path.exists(caminho_pdf): 
            ...
            logging.info(f"Backup PDF ({tipo}) -> {os.path.join(destino_dir, os.path.basename(caminho_pdf))}") 
    except Exception as ex: 
...
def formatar_cpf(event, entry): 
    ...
    novo = '' 
    for i, c in enumerate(testo:=texto): 
        if i == 3 or i == 6: 
            novo += '.' 
        if i == 9: 
            novo += '-' 
...
def abrir_sistema_com_logo(username, login_win): 
    ...
    def fechar_caixa(): 
        ...
        if messagebox.askyesno("Fechar Caixa", f"Total do dia: R$ {total:.2f}\nDeseja fechar o caixa?"): 
            ...
            # --- Backups automáticos ao fechar caixa --- 
            try: 
                backup_banco() 
    ...
    def _show_toast_backup(text: str, level: str = "info"): 
        try: 
            ...
            if hasattr(root, '_toast_frame') and root._toast_frame and root._toast_frame.winfo_exists(): 
                try: 
                    root._toast_frame.destroy() 
                except Exception: 
                    pass 
            root._toast_frame = tk.Frame(root, bg=bg, highlightthickness=0) 
            lbl = tk.Label(root._toast_frame, text=text, bg=bg, fg=fg, font=("Segoe UI", 10, "bold")) 
            lbl.pack(padx=10, pady=6) 
            root._toast_frame.place(relx=0.5, rely=0.02, anchor='n') 
            root.after(5000, lambda: root._toast_frame.destroy() if root._toast_frame.winfo_exists() else None) 
        except Exception: 
            pass 
