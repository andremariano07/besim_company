...
# ====== MIGRAÇÃO: garantir coluna is_admin existe ======
...
def setup_global_exception_handlers(tk_root: tk.Misc):
    """Configura tratadores globais de exceção (Python e callbacks Tkinter)."""
...
def _extrair_repo(zip_path, temp_dir):
    destino = os.path.join(temp_dir, "src")
    with zipfile.ZipFile(zip_path, "r") as z:
        z.extractall(destino)
    # diretório raiz do zip: <repo>-<branch>
    return next(os.scandir(destino)).path

def _copiar_arquivos(origem, destino):
...
def check_and_update(app_version, base_path):
    try:
        versao_remota = _obter_versao_remota()
    except Exception:
        return False
...
def abrir_sistema_com_logo(username, login_win):
    ...
    btn_cad_prod = ttk.Button(btn_frame_est, text="Cadastrar", command=lambda: cadastrar_produto())
    btn_edit_load_prod = ttk.Button(btn_frame_est, text="Editar (carregar)", command=lambda: carregar_produto_selecionado())
    btn_save_edit_prod = ttk.Button(btn_frame_est, text="Salvar Edição", command=lambda: salvar_edicao_produto())
    btn_del_prod = ttk.Button(btn_frame_est, text="Excluir", command=lambda: excluir_produto())
